<template>
  <div class="container">
    ...
    {{mesaj}}
    <div v-if="secilen_seferler.length">
      <h2>Sefer Listesi</h2>
      <table class="table table-hover">
        <tr v-for="(item, index) in secilen_seferler">
          <td>Kalkış Saati
...
export default {
  ...
  methods:{
    sefer_ara(){
      this.isLoading=true;
      this.secilen_seferler=this.seferler.filter(x =>
      ...
      if(this.secilen_seferler.length===0){
    ...
    sefer_sec(sefer_id){
      ...
      //koltukSecimi sayfasına yönlendirme yaptık
    }
