<template>
    ...
    <table class="table table-bordered text-center">
        ...
        <button class="btn btn-success float-right" @click="yolcu_bilgileri" v-if="secilen_koltuklar.length>0">Koltukları Seç</button>
    </div>
</template>
<script>
    ...
    export default {
        ...
        methods:{
            fetchKoltuklar(){
                ...
                this.secilen_seferler=this.seferler.find(x => x.id === sefer_id);
            ...
            koltuk_sec(koltuk_id){
                ...
                if(koltuk.durum===0)
                    this.secilen_koltuklar = this.secilen_koltuklar.filter(k => k.id !== koltuk.id);
                else
